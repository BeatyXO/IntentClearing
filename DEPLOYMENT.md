...
The dedicated repository is [BeatyXO/IntentClearing](https://github.com/BeatyXO/IntentClearing), configured on `main`. The separate `BeatyXO/Corroborate` repository was not changed. A push of `.github/workflows/ci.yml` previously required the GitHub OAuth `workflow` scope. The current local `gh auth status` reports the BeatyXO token invalid; a fresh GitHub CLI device login requesting `repo,workflow` is waiting for the account owner to sign in and authorize it. Consequently this checkout's latest commits are not verified on GitHub and remote CI has not run. No final GitHub commit SHA or CI result is claimed.
